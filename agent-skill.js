...
- <select> and <option> elements have native browser rendering that ignores most CSS in some browsers, but background-color and color on both select.form-select and select.form-select option must always be set together in dark mode. If the select background is dark, set color on both the select element and its option elements explicitly — otherwise options will render with a browser-default light background and dark text, making the selected value invisible against the dark select background. Example: \`select.form-select, select.form-select option { background-color: #1e1e1e; color: #e0e0e0; }\`
